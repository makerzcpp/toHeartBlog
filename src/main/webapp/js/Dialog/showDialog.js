...
function showInfo2(msg, sd_width, title, fn, timeout) {
	var sd_width = sd_width ? sd_width : 320;
	showDialog("info", msg, title, sd_width);
	$("#SD_confirm").unbind("click");
	if(fn && timeout) {
		st = setTimeout(function(){
			sd_remove();
...
/**
 * confirm [确认模式]
 * 应用场景：主要用于确认取消的选择,示例如下:
 * showConfirm("请问是否确认删除？",function(){
 * 	 //当点击确认按钮后,执行下列代码
 *   //填充删除操作的代码
 * });
 * @param msg

...
scrollpic.js
*/
function ScrollPic(scrollContId, arrLeftId, arrRightId, dotListId, listType ,hotId) {
	this.scrollContId = scrollContId;
	this.arrLeftId = arrLeftId;
	this.arrRightId = arrRightId;
	this.arrsVisible = true;
	...
	this.frameWidth = 0;
	this.speed = 10;
	this.space = this.ieVersion == (7||8) ? 100 :10;
	this.upright = false;
	this.pageIndex = 0;
	this.autoPlay = true;
	this.autoPlayTime = 5;
	...
	this.arrLeftDisabled = "arrLeftDisabled";
	this.arrRightDisabled = "arrRightDisabled";
	this.hotId = hotId;
};
ScrollPic.prototype = {
	version: "1.45",
	...
	initialize: function() {
		...
		if (this.hotId) {
			this.hotArea = this.$(this.hotId);
			if(this.hotArea){
				this.addEvent(this.hotArea, "mouseover", function() {
					if(thisTemp.alObj && thisTemp.arObj && !this.arrsVisible){
						thisTemp.alObj.style.display = 'block';
						thisTemp.arObj.style.display = 'block';
					}
				});
				this.addEvent(this.hotArea, "mouseout", function() {
					if(thisTemp.alObj && thisTemp.arObj && !this.arrsVisible){
						thisTemp.alObj.style.display = 'none';
						thisTemp.arObj.style.display = 'none';
					}
				});
			}
		}
		...
		if (this.arrLeftId) {
			...
			if (this.alObj) {
				if (!this.circularly) {
					this.addClass(this.alObj, 'arrLeftDisabled');
				}
				if (!this.arrsVisible) {
					this.alObj.style.display = 'none';
				}
	...
	move: function(num, quick) {
		...
		if (thisMove > 0) {
			if (this.circularly) {
				...
				this.removeClass(this.alObj, 'arrLeftDisabled');
				if (this.scDiv[this._scroll] + thisMove >= this.lDiv01[this._sWidth] - this.frameWidth) {
					this.scDiv[this._scroll] = this.lDiv01[this._sWidth] - this.frameWidth;
					this._state = "ready";
					theEnd = true;
					this.addClass(this.arObj, 'arrRightDisabled');
				} else {
					this.scDiv[this._scroll] += thisMove;
				}
			}
			...
			if (this.circularly) {
				...
				this.removeClass(this.arObj, 'arrRightDisabled');
				if (this.scDiv[this._scroll] + thisMove <= 0) {
					this.scDiv[this._scroll] = 0;
					this._state = "ready";
					theEnd = true;

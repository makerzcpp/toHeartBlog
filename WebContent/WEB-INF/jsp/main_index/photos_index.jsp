				...
				<c:forEach var="box" items="${list}" varStatus="ls">	
					...
					<c:if test="${ls.index%2==0}">				
						...
						<div class="p1_box left cl1">
							...
							<a href="${BASE}/photo/photoslist/${box.boxId}" class="bot">${box.boxName} <span>${box.imgNum} <br>images</span></a>
						</div>
						</c:if>
						...
						<div class="p1_box left cl3 pos1">
							<div class="type"></div>
							<img src="${BASE}${box.imgHead}" alt="">
							<a href="${BASE}/photo/photoslist/${box.boxId}" class="bot">${box.boxName}<span><strong class=" fa-angle-left fa"></strong></span></a>
						</div>
						</c:if>
					</c:if>	
					</div>
					...
					<c:if test="${box.boxType==0}">
						...
						<div class="p1_box right cl3">
							<div class="type"></div>
							<img src="${BASE}${box.imgHead}" alt="">
							<a href="${BASE}/photo/photoslist/${box.boxId}" class="bot">${box.boxName}<span><strong class=" fa-angle-right fa"></strong></span></a>
						</div>
						</c:if>
						</c:if>
					</div>
